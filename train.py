...
class ClusterDataset(Dataset):
    ...
    Each row of the input CSV is expected to have 11 features:
        - First 5 columns: avgWire values (normalized by 112)
        - Next 5 columns: slope values
        - Last column: superlayer of missing cluster
    ...
    def __init__(self, csv_files):
        ...
        # Split features
        X_avg = df.iloc[:, :5].values.astype(np.float32) / 112.0
        X_slope = df.iloc[:, 5:10].values.astype(np.float32)
        X_superlayer = df.iloc[:, [-2]].values.astype(np.float32) / 6.0
        self.X = np.concatenate([X_avg, X_slope, X_superlayer], axis=1)
